...
const Navbar: React.FC<NavbarProps> = ({ menuItems, brandName }) => {
  ...
  return (
    <header className="bg-white w-full z-50 shadow">
      ...
      {/* Mobile sidebar menu */}
      <div
        className={`fixed top-0 right-0 h-full w-64 bg-primary text-background shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${
          mobileOpen ? "translate-x-0" : "translate-x-full"
        } md:hidden`}
      >
        <div className="p-4 border-b border-background flex justify-between items-center">
          <div className="flex items-start flex-col ">
            <span className="text-2xl font-semibold text- tracking-[-0.5px]">
              REX EIGHT IT
            </span>
          </div>
          <button
              ...
              item.submenu ? (
                    ...
                    {item.submenu.map((subitem, idx) => (
                      <li key={idx}>
                        <Link
                          href={subitem.path || "#"}
                          onClick={toggleMobileMenu}
                          className={`block px-2 py-1 rounded hover:bg-foreground ${
                            isActive(subitem.path)
                              ? "bg-gray-700 font-semibold"
                              : ""
                          }`}
                ...
                <li key={i}>
                  <Link
                    href={item.path || "#"}
                    onClick={toggleMobileMenu}
                    className={`block px-2 py-2 rounded hover:bg-foreground ${
                      isActive(item.path) ? "bg-secondary font-semibold" : ""
                    }`}
                  >
                    {item.label}
                  </Link>
