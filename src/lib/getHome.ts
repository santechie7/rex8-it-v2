import { promises as fs } from 'fs';
import path from 'path';
import { imageMap } from './imageMap';
import { svgMap } from './svgMap';

export async function getHome() {
  let data;

  // try {
  //   // Try API first (if URL is configured)
  //   if (process.env.NEXT_PUBLIC_API_URL) {
  //     const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/home', {
  //       cache: 'no-store'
  //     });
      
  //     if (res.ok) {
  //       data = await res.json();
  //     }
  //   }
  // } catch (error) {
  //   console.log('API failed, using local JSON');
  // }

  // Fallback to local JSON if API failed
  if (!data) {
    const filePath = path.join(process.cwd(), 'src/public/home.json'); // Fixed: removed 'src/'
    const fileData = await fs.readFile(filePath, 'utf8');
    data = JSON.parse(fileData);
  }

  // Map image key to actual import
  if (data.heroData?.imageKey) {
    data.heroData.imageKey = imageMap[data.heroData.imageKey];
  }
  if (data.problem?.imageKey) {
    data.problem.imageKey = imageMap[data.problem.imageKey];
  }

  if (data.showcaseData?.items) {
    data.showcaseData.items = data.showcaseData.items.map((item: any) => ({
      ...item,
      svgContent: item.svgContent ? svgMap[item.svgContent] : null,
    }));
  }

  return data;
}
